...
class GameDAO extends Connection
{
    ...
    public function getIDsArray($limit, $orderBy, $order, $storeFilter)
    {
        $allowed = array('id_game', 'rating_count'); //valores de orderby permitidos
        //como pdo filtra somente values, verifica manualmente se nao ha tentativa de sql injection
        if (!in_array($orderBy, $allowed)) $orderBy = 'rating_count';
        $query = null;
        if (!$storeFilter) { //se nao houver filtro de lojas do usuario
            $query = $this->pdo->prepare('SELECT d.id_game  FROM deals AS d INNER JOIN 
            games AS g ON (g.id = d.id_game) WHERE d.current_deal = 1 GROUP BY d.id_game ORDER BY '
                . $orderBy . ' ' . $order . ' LIMIT :limit');
        } else { //se houver filtro de lojas do usuario
            $query = $this->pdo->prepare('SELECT d.id_game FROM deals AS d INNER JOIN games AS g ON 
            (g.id = d.id_game) WHERE d.current_deal = 1 AND d.id_store IN (:stores) GROUP BY d.id_game 
            ORDER BY ' . $orderBy . ' ' . $order . ' LIMIT :limit');
            $query->bindValue(':stores', $storeFilter);
        }
    ...
    public function searchGame($name, $limit, $storeFilter)
    {
        //caso o termo de pesquisa seja menor q 4 caracteres, utiliza query LIKE termo%
        //caso tenha 4 caracteres ou mais, utilzia query LIKE %termo%
        ...
        $query = null;
        if (!$storeFilter) { //se nao houver filtro de lojas do usuario
            //busca jogos sem filtro de lojas
            $query = $this->pdo->prepare('SELECT id FROM games WHERE (name LIKE :search OR alt_name_1 LIKE :search OR 
            alt_name_2 LIKE :search) AND plain IS NOT NULL AND active = 1 ORDER BY rating_count DESC LIMIT :limit');
        } else { //se houver filtro de lojas do usuario
            //query com filtro de lojas, soh busca jogos q possuam deals ativas nas lojas informadas 
            $query = $this->pdo->prepare('SELECT g.id FROM games AS g INNER JOIN deals AS d ON (g.id = d.id_game) 
            WHERE d.id_store IN (:stores) AND d.current_deal = 1 AND (g.name LIKE :search OR g.alt_name_1 LIKE :search 
            OR g.alt_name_2 LIKE :search) AND g.plain IS NOT NULL AND g.active = 1 GROUP BY g.name ORDER BY 
            g.rating_count DESC LIMIT :limit');
            $query->bindValue(':stores', $storeFilter);
        }
        $query->bindValue(':search', $search);
        $query->bindValue(':limit', $limit, \PDO::PARAM_INT);
